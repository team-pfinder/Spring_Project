...
/* 
	// 잘못입력했거나 필수사항을 입력하지 않았을 경우 다음페이지로 넘어가지못하는 function
	function formCheck() {
	
	  var f = $("#inputBasicInfo");
	  //var flag;
	  
	  // form[id].input[id].value==""
	  if (f.$("#inputLocName").value == "")// || + 제대로입력하지 않았을 경우 ) {
	
	    alert("값을 입력해 주세요.");
	
	    f.$("#inputLocName").focus();
	
	    // flag = function's return value;
	    // return flag;
	    return false;
	
	  }
	    */
	  // 추가로 시설안내, 주의사항의 경우 추가된 정보를 확인한다.
...
<div class="container">
   ...
   <p class="mb-4"> 기본정보를 입력하세요. <a target="_blank" href="#">이전으로</a>.</p>
	...
	<form style="width: 80%; margin: 120px;" id="inputBasicInfo" enctype="multipart/form-data"
		  action="${pageContext.request.contextPath}/inputContactinfo.jsp" method="post"><!--onsubmit="handOver()" -->
