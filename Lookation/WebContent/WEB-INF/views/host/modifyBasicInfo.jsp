...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		// 공간유형 체크
		//$('#inputLocType').val() = ${basicInfoList.loc_type}
		//$("#inputLocType option:selected").val();
		//var type = "${basicInfoList.loc_type}";
		//$('#inputLocType').val(type).prop("selected", true);
		...
		setInputLength($('#inputIntro'), '공간 소개', 5, 1000);
		...
		$('#submitButton').click(function() {
			...
			else if (tIntro.length < 5 || tIntro.length > 1000) {
				
				alert("공간 소개는 5자~1000자로 입력해야합니다.");
				$("#inputIntro").focus();
...
<div class="container">
   ...
   <!-- Page Heading -->
   <h1 class="mb-2 text-gray-800">기본정보 수정</h1>
   <p class="mb-4"> 기본정보를 수정하세요. <a target="_blank" href="#"></a></p>
      ...
      <div class="card shadow mb-4">
         <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-default">기본정보 수정</h6>
         </div>
	...
	<form style="width: 80%; margin: 120px;" id="inputBasicInfo" enctype="multipart/form-data"
		  action="modifybasicinfo.action?loc_code=${loc_code }" method="post">
	
	<%-- <c:forEach var="loc" items="${basicInfoList }">
	 --%>
	...
	<%-- 	
		<div>
			...
			<br><br>
			<select id="inputLocType" name="inputLocType" class="form-control" disabled="disabled">
				<option value="">[==공간 유형을 선택하세요.==]</option>
				<option value="${basicInfoList.loc_type}" <c:if test="${basicInfoList.loc_type eq '브라이덜샤워'}">selected</c:if>>브라이덜샤워</option>
				<option value="${basicInfoList.loc_type}" <c:if test="${basicInfoList.loc_type eq '루프탑'}">selected</c:if>>루프탑</option>
				<option value="${basicInfoList.loc_type}" <c:if test="${basicInfoList.loc_type eq '엠티공간'}">selected</c:if>>엠티공간</option>
				<option value="${basicInfoList.loc_type}" <c:if test="${basicInfoList.loc_type eq '파티룸'}">selected</c:if>>파티룸</option>
			</select>
	...
	<br><br><br>
		...
		<div>
			...
			<input type="text" class="form-control"
				   ...
				   value="${basicInfoList.loc_short_intro}">
	...
	<br><br><br>
		...
		<div>
			...
			<textarea class="form-control" 
					  ...
					  id="inputIntro" name="inputIntro" cols="40" rows="5">${basicInfoList.loc_intro}</textarea>
		  	<span style="font-weight: bold;"></span>
		</div>
	...
	<br><br><br>
		...
		<div>
			...
			<input type="text" class="form-control"  
					...
					value="${basicInfoList.loc_detail_addr }">
					<%-- value = "${loc.loc_detailAddr }" --%>
